...
def consumir_insumo():
    if len(nome_insumo.get()) < 2 or len(lote_insumo.get()) < 1 or len(qtde_insumo.get()) < 1:
        ...
        # escrever na caixa de texto
        caixa_texto.insert("1.0", f"Nome, quantidades e lote do insumo inválidos!")
        return
    ...
    caixa_texto.delete("1.0", END)
        ...
        # escrever na caixa de texto
    caixa_texto.insert("1.0", f"{nome_insumo.get()} : {qtde_insumo.get()} unidades consumidos com sucesso")
...
def visualizar_insumo():
    ...
    for Id, Produto, Quantidade, DataValidade, Lote in valores:
        texto = texto + f"""
        Produto: {Produto}
